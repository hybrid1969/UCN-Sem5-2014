		...
		for (int h = 0; h < tHeight; h++) {
			GUILayout.BeginHorizontal();
				...
				for (int w = 0; w < tWidth; w++) {
					terrains[counter] = (Terrain)EditorGUILayout.ObjectField(terrains[counter++], typeof(Terrain), true, GUILayout.Width(112));
					GUILayout.Space(5);
				}
			GUILayout.EndHorizontal();
			GUILayout.Space(9);
...
#if UNITY_3_4
		GUI.Label( new Rect(5, y-4, 20, 20), "Z");
#else
